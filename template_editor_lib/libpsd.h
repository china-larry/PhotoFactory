...
typedef struct _editor_parameter
{
	...
	int         layer_or_photo; // 对图层操作为0，对人像照片操作为1
	...
	IplImage *  customer_image; // 客照的opencv对象
...
typedef struct _prepare_layerinfo
{
	int layer_mask_top;
	int layer_mask_bottom;
	int layer_mask_left;
	int layer_mask_right;
	psd_int layer_top;
	psd_int layer_bottom;
	psd_int layer_left;
	psd_int layer_right;
	psd_int layer_height;
	psd_int layer_width;
}prepare_layerinfo;

typedef struct _present_layerinfo
{
	int layer_mask_top;
	int layer_mask_bottom;
	int layer_mask_left;
...
IplImage* dst_zoom_tmp;
IplImage* dst_rotate_tmp;
IplImage* clipp_image;
char* catalog;
float context_scale;
struct rebuild    // 判断是解析还是重建过程，rebuilding为0是解析，为1是重建
{
	psd_int rebuilding;
}rb;
...
struct level           //色阶
{
	psd_ushort					input_floor[4][10];		// (0...253)
	psd_ushort					input_ceiling[4][10];		// (2...255)
	psd_ushort					output_floor[4][10];		// 255). Matched to input floor.
	psd_ushort					output_ceiling[4][10];		// (0...255)
	psd_float					gamma[4][10];				// Short integer from 10...999 representing 0.1...9.99. Applied to all image data.
	psd_int						haolevel;
}le;

struct huesaturation             //色相饱和度
{
	psd_int hue_value[5];
	psd_int saturation_value[5];
	psd_int lightness_value[5];
	psd_int haoh;
}hs;


struct selectivecolor            //可选颜色
{
	psd_ushort					correction_method1[10];		// 0 = Apply color correction in relative mode; 1 = Apply color correction in absolute mode.
	psd_short					cyan_correction1[5][10];	// Amount of cyan correction. Short integer from –100...100.
	psd_short					magenta_correction1[5][10];	// Amount of magenta correction. Short integer from –100...100.
	psd_short					yellow_correction1[5][10];	// Amount of yellow correction. Short integer from –100...100.
	psd_short					black_correction1[5][10]; 	// Amount of black correction. Short integer from –100...100.
	psd_int                     haosc;
}sc;

 struct threshold                //阈值
{
	psd_ushort					level[5];					// (1...255)
	psd_int						haots;
} ts;

struct brightness_contrast      //亮度对比对
{
	psd_int brightness[10];
	psd_int contrast[10];
	psd_int haobc;
}bc;

struct colorblance				//色彩平衡
{
	psd_int c_r[3][10];
	psd_int m_g[3][10];
	psd_int y_b[3][10];
...
struct posterize                //色调分离
{
	psd_ushort					levels[10];					// (2...255)
	//psd_uchar					lookup_table[256];
	psd_int						haopost;
}posterize;

struct curvesdata               //曲线
{
	psd_ushort					channel_index[5][10];		// Before each curve is a channel index.
	psd_ushort					point_count[5][10];		// Count of points in the curve (psd_short integer from 2...19)
	psd_ushort					output_value[5][19][10];	// All coordinates have range 0 to 255
	psd_ushort					input_value[5][19][10];
	psd_int						haocurves;
} curves;


struct channelmixer             //通道混合器
{
	psd_bool					monochrome[10];             //单色
	psd_short					red_cyan[4][10];			// RGB or CMYK color plus constant for the mixer settings. 4 * 2 bytes of color with 2 bytes of constant.
	psd_short					green_magenta[4][10];		// (-200...200)
	psd_short					blue_yellow[4][10];
	psd_short					black[4][10];
	psd_short					constant[4][10];
	psd_int						haocm;
}cm;

struct photofilter              //照片滤镜
{
	psd_int						x_color;		// 4 bytes each for XYZ color
	psd_int						y_color;
	psd_int						z_color;
	psd_int						density;		// (1...100)
	psd_bool					preserve_luminosity; //保留明度
} pf;

struct newmask
{
	psd_int						top[100];			// Rectangle enclosing layer mask: Top, left, bottom, right
	psd_int						left[100];
	psd_int						bottom[100];
	psd_int						right[100];
	psd_int						width[100];
	psd_int						height[100];
	psd_color_component			default_color[100];	// 0 or 255
	psd_bool					relative[100];		// position relative to layer
	psd_bool					disabled[100];		// layer mask disabled
	psd_bool					invert[100];			// invert layer mask when blending
	psd_color_component *		mask_data[100];
	psd_int						haomask;
...
/////////////////////////////////////////////////////////////////////////// added by xiaorui
	psd_int eftag;				//混合选项的标号
	//psd_int colroadjust;      //色彩调整的标号
	psd_int being_clipping;     //被关联的一层   “1为被关联”
	psd_int layer_num;			//图层号
	IplImage* customer_image;   //
	IplImage* src_image;
	IplImage* mask_image;
	prepare_layerinfo *         layer_src_info;
	prepare_layerinfo *         layer_editor_prepare_info;
	prepare_layerinfo *         layer_editor_present_info;
};

//typedef struct _psd_layer_record		psd_layer_recordss;
//psd_layer_recordss * layeradd;

//(((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((
//typedef struct _psd_layer_record2		psd_layer_record2;
//struct _psd_layer_record2
//{
//	psd_layer_type				layer_type;
//	psd_int						top;				// Rectangle containing the contents of the layer. Specified as top, left, bottom, right coordinates
//	psd_int						left;
...
//	psd_layer_record *			group_layer;
///////////////////////////////////////////////////////////////////////////// added by xiaorui
//	psd_int eftag[11];			//混合选项的标号
//	psd_int colroadjust[12];    //色彩调整的标号
//
//}layeradd;
...
typedef struct _psd_context
{
    ...
    IplImage* thumbnail_image;   //
	float						contextscale;

} psd_context;


psd_context * context3;
...
psd_status psd_image_blend(psd_context * context, psd_int left, psd_int top, psd_int width, psd_int height,editor_parameter *);
psd_status psd_image_blend2(psd_context * context, psd_int left, psd_int top, psd_int width, psd_int height);
